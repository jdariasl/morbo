...
def Pena_func(X):
    ...
    return problem  # torch.cat([problem, Pena_constr_val(X)], dim=-1)
...
def Pena_constr(X):
    ...
    res.append(torch.matmul(X, -CF) + 6 >= 0)
    res.append(torch.matmul(X, Ca) - 0.6 >= 0)
    res.append(torch.matmul(X, -Ca) + supbd >= 0)
    res.append(torch.matmul(X, AP) - 0.15 >= 0)
    res.append(torch.matmul(X, -AP) + supbd >= 0)
    res.append(torch.matmul(X, DM) - 87 >= 0)
    res.append(torch.matmul(X, -DM) + 95 >= 0)
    res.append(torch.matmul(X, CP) - 18 >= 0)
    res.append(torch.matmul(X, -CP) + 21 >= 0)
    res.append(torch.matmul(X, MC) - 0.475 >= 0)
    res.append(torch.matmul(X, -MC) + supbd >= 0)
    res.append(torch.matmul(X, Tp) - 0.171 >= 0)
    res.append(torch.matmul(X, -Tp) + supbd >= 0)
    res.append(torch.matmul(X, T) - 0.627 >= 0)
    res.append(torch.matmul(X, -T) + supbd >= 0)
    res.append(torch.matmul(X, ones) - 0.99 >= 0)
    res.append(torch.matmul(X, -ones) + 1.01 >= 0)

    return torch.stack(res, dim=-1).all(dim=-1)
